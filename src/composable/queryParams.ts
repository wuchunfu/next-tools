...
function useQueryParamOrStorage<T>({ name, storageName, defaultValue }: { name: string, storageName: string, defaultValue: T }): ReturnType<typeof ref<T>> {
  const type = typeof defaultValue
  const transformer = transformers[type as keyof typeof transformers] ?? transformers.string

  const storageRef = useStorage<T>(storageName, defaultValue)
