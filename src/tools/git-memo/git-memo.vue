...
const memoComponent = computed(() => {
  ...
  const target = localeToFile[lang] ?? 'en'
  return defineAsyncComponent(async () => {
    try {
      return (await import(`./memo/git-memo.content.${target}.md`)).default
    }
    catch (e) {
      console.warn('Fallback to English git memo content', e)
      return (await import('./memo/git-memo.content.en.md')).default
    }
  })
});
</script>
