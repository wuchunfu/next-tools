...
:--|:--
`.` ou `[^\n\r]` | n'importe quel caractère *sauf* un saut de ligne ou un retour chariot
`[A-Za-z]` | alphabet
...
- Considérez un ensemble de caractères comme une opération `OU` sur les caractères individuels qui sont enfermés entre les crochets.
- Utilisez `^` après le `[` d'ouverture pour "néguer" l'ensemble de caractères.
...
- Un `-` doit être échappé uniquement s'il se produit entre deux lettres ou deux chiffres.
...
- Comment fonctionne la correspondance de limite de mot :
...
Expression | Description
:--|:--
`foo\|bar` | correspondre à `foo` ou `bar`
`foo(?=bar)` | correspondre à `foo` s'il est avant `bar`
`foo(?!bar)` | correspondre à `foo` s'il n'est *pas* avant `bar`
`(?<=bar)foo` | correspondre à `foo` s'il est après `bar`
`(?<!bar)foo` | correspondre à `foo` s'il n'est *pas* après `bar`

### Regroupement et capture

Expression | Description
:--|:--
`(foo)` | groupe de capture ; correspondre et capturer `foo`
`(?:foo)` | groupe non capturant ; correspondre à `foo` mais *sans* capturer `foo`
`(foo)bar\1` | `\1` est une référence arrière au 1er groupe de capture ; correspondre à `foobarfoo`

- Les groupes de capture ne sont pertinents que dans les méthodes suivantes :
    - `string.match(regexp)`
    - `string.matchAll(regexp)`
    - `string.replace(regexp, callback)`
- `\N` est une référence arrière au groupe de capture `N-ième`. Les groupes de capture sont numérotés à partir de 1.
